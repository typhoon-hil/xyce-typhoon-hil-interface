...
class VoltageSource(TwoTerminal):
    ...
    def create_new_format_properties_dict(self, tse_properties, tse_component):
        ...
        if self.type == "VDC":
            transient_spec = tse_properties["voltage"]
        elif self.type in ["I_MEAS", "I_MEAS_OUT"]:
            transient_spec = "0"
        elif self.type == "VSIN":
            transient_spec = f"SIN {' '.join(source_tr_spec_properties)}"
        elif self.type == "VPULSE":
            transient_spec = f"PULSE {' '.join(source_tr_spec_properties)}"
        elif self.type == "VEXP":
            transient_spec = f"EXP {' '.join(source_tr_spec_properties)}"
        elif self.type == "VTRI":
            V0 = tse_properties["vmin"]
            V1 = tse_properties["vmax"]
            T1 = float(tse_properties["trise"])
            T2 = float(tse_properties["tfall"])
            transient_spec = f"PWL 0S {V0} {str(T1)} {str(V1)} {str(T1 + T2)} {V0} R=0"
    ...
    def create_measurements(self, circuit, nodes, tse_properties, tse_component):
        ...
        tse_properties["meas_i"] = "True" if self.type in ["I_MEAS", "I_MEAS_OUT"] else tse_properties["meas_i"]
