...
class BJT(ModelBased):
    ...
    def define_nodes(self, tse_properties, tse_component):
        ...
        nodes = [
            XYCE_NODE_PREFIX + str(tse_component.terminals["emit"].node.name),
            XYCE_NODE_PREFIX + str(tse_component.terminals["base"].node.name),
            XYCE_NODE_PREFIX + str(tse_component.terminals["col"].node.name)
        ]
...
class SubcircuitBJT(SubcircuitBased):
    """ Converted component class. """
