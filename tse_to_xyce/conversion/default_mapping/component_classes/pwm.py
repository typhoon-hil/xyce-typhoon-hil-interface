...
class PulseWidthModulator(SubcircuitBased):
    ...
    def create_new_format_properties_dict(self, tse_properties, tse_component):
        ...
        new_format_properties = {
            "vmax": tse_properties["vmax"],
            "TS": 1 / float(tse_properties['freq']),
            "DT": 0.01 * float(tse_properties['deadtime'])
        }
    ...
    def create_component_model(self, tse_properties, tse_component):
        ...
        if not component_model:
            component_model = ComponentModel(self.circuit, model_name,
                                             file_path=os.path.join(XYCE_INCLUDED_MODELS_PATH, "pwm.lib"))
    ...
    def define_nodes(self, tse_properties, tse_component):
        ...
        nodes = [
            XYCE_NODE_PREFIX + str(tse_component.terminals["duty"].node.name),
            XYCE_NODE_PREFIX + str(tse_component.terminals["Comp Out"].node.name),
