...
class MeasureWithOutput(SubcircuitBased):
    ...
    def create_component_model(self, tse_properties, tse_component):
        ...
        if self.type == "I_MEAS_OUT":
            model_name = "i_meas_out"
        elif self.type == "V_MEAS_OUT":
            model_name = "v_meas_out"
        ...
        if not component_model:
            component_model = ComponentModel(self.circuit, model_name,
                                             file_path=os.path.join(XYCE_INCLUDED_MODELS_PATH, f"meas_with_outputs.lib"))
    ...
    def define_nodes(self, tse_properties, tse_component):
        ...
        nodes = [
            ...
            XYCE_NODE_PREFIX + str(tse_component.terminals["signal_out"].node.name)
        ]
    ...
    def create_measurements(self, circuit, nodes, tse_properties, tse_component):
        ...
        if self.type == "I_MEAS_OUT":
            i_meas = CurrentMeasurement(circuit, self, measured_subsystem_component="V_MEAS")
